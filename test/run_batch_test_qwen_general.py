"""Batch test runner for CondVQA using Qwen2.5-VL-7B - general questions (tc201-tc300)."""
...
def load_test_cases(csv_path):
    """Load general question test cases from CSV file."""
    test_cases = []
...
def run_batch_test_qwen_general(
    csv_path=None,
    video_dir=None,
    model=None,
    processor=None,
    ...
    """
    Run all general question test cases from CSV using Qwen2.5-VL-7B.
    ...
    if output_dir is None:
        output_dir = os.path.join(script_dir, 'batch_results_qwen_general')
    ...
    # ── Summary Report ──
    print(f"\n{'='*70}")
    print("QWEN GENERAL BATCH TEST RESULTS SUMMARY")
    print(f"{'='*70}\n")
    ...
    results_csv_path = os.path.join(output_dir, 'batch_results_qwen_general.csv')
