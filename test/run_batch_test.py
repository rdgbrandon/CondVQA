...
def run_batch_test(
    ...
    """
    Run all test cases from CSV and compare predictions to expected answers.
    ...
    for i, tc in enumerate(test_cases):
        ...
        try:
            conditional_results = conditional_query_vqa_interpret(
                ...
                output_dir=tc_output_dir if tc_output_dir else 'conditional_query_analysis',
                include_text_attribution=False
            )

            # Extract result for this question
            question_key = tc['question']
            if question_key in conditional_results:
                result = conditional_results[question_key]
                if 'error' in result:
                    prediction = 'ERROR'
                    confidence = 0.0
