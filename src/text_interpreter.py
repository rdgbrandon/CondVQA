    ...
    for idx, question in enumerate(questions, 1):
        ...
        if mode in ['text', 'both']:
            ...
            if show_visualizations:
                save_path = f"{save_dir}/text_attribution_q{idx}.png" if save_dir else None
                visualize_text_attributions(text_result, save_path)

        if mode in ['joint', 'both']:
            print("\nCOMPUTING JOINT IMAGE+TEXT ATTRIBUTIONS")
            ...
            if show_visualizations:
                save_path = f"{save_dir}/joint_attribution_q{idx}.png" if save_dir else None
